...
class enrol_keyusermanual_enrol_users_form extends moodleform {
    ...
    public function definition() {
        ...
        $options = array(
            'ajax' => 'enrol_keyusermanual/form-potential-user-selector',
            'multiple' => true,
            'courseid' => $course->id,
            'enrolid' => $instance->id,
        ...
        $mform->addElement('autocomplete', 'userlist', get_string('selectusers', 'enrol_keyusermanual'), array(), $options);

        // Confirm the user can search for cohorts before displaying select.
        if (has_capability('local/keyuser:cohortmanage', $context) || has_capability('local/keyuser:cohortview', $context)) {
            // Check to ensure there is at least one visible cohort before displaying the select box.
            // Ideally it would be better to call external_api::call_external_function('keyuser_core_cohort_search_cohorts')
            ...
            $availablecohorts = keyuser_cohort_get_cohorts($context->id, 0, 1, '');
            $availablecohorts = $availablecohorts['cohorts'];
            if (!($context instanceof context_system)) {
                $availablecohorts = array_merge($availablecohorts,
                    keyuser_cohort_get_available_cohorts($context, COHORT_ALL, 0, 1, ''));
            }
            if (!empty($availablecohorts)) {
                $options = ['contextid' => $context->id, 'multiple' => true];
                $mform->addElement('keyusercohort', 'cohortlist', get_string('selectcohorts', 'enrol_keyusermanual'), $options);
            }
        }
