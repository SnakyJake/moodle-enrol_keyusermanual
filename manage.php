...
$instance = $DB->get_record('enrol', array('id'=>$enrolid, 'enrol'=>'keyusermanual'), '*', MUST_EXIST);
$course = $DB->get_record('course', array('id'=>$instance->courseid), '*', MUST_EXIST);
$context = context_course::instance($course->id, MUST_EXIST);

require_login($course);
$canenrol = has_capability('enrol/keyusermanual:enrol', $context);
$canunenrol = has_capability('enrol/keyusermanual:unenrol', $context);
...
if (!$enrol_keyusermanual = enrol_get_plugin('keyusermanual')) {
    throw new coding_exception('Can not instantiate enrol_keyusermanual');
}

$instancename = $enrol_keyusermanual->get_instance_name($instance);

$PAGE->set_url('/enrol/keyusermanual/manage.php', array('enrolid'=>$instance->id));
$PAGE->set_pagelayout('admin');
$PAGE->set_title($enrol_keyusermanual->get_instance_name($instance));
...
$potentialuserselector = new enrol_keyusermanual_potential_participant('addselect', $options);
$potentialuserselector->viewfullnames = $viewfullnames;
$currentuserselector = new enrol_keyusermanual_current_participant('removeselect', $options);
...
if (empty($extendbase)) {
    if (!$extendbase = get_config('enrol_keyusermanual', 'enrolstart')) {
...
$basemenu[4] = get_string('now', 'enrol_keyusermanual') . ' (' . userdate($now, get_string('strftimedatetimeshort')) . ')';
...
if ($canenrol && optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstoassign)) {
        foreach($userstoassign as $adduser) {
            ...
            $enrol_keyusermanual->enrol_user($instance, $adduser->id, $roleid, $timestart, $timeend);
        }
...
if ($canunenrol && optional_param('remove', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if (!empty($userstounassign)) {
        foreach($userstounassign as $removeuser) {
            $enrol_keyusermanual->unenrol_user($instance, $removeuser->id);
        }
    ...
    <tr>
      ...
      <td id="buttonscell">
          <div id="addcontrols">
              ...
              <p><label for="menuroleid"><?php print_string('assignrole', 'enrol_keyusermanual') ?></label><br />
