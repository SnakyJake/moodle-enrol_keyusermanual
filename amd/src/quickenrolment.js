 ...
 *
 * @module     enrol_keyusermanual/quickenrolment
...
const Selectors = {
    ...
    triggerButtons: ".enrolusersbutton.enrol_keyusermanual_plugin [type='submit']",
...
const getBodyForContext = contextId => {
    return Fragment.loadFragment('enrol_keyusermanual', 'enrol_users_form', contextId, {});
};
...
const showModal = (dynamicTable, contextId) => {
    const pendingPromise = new Pending('enrol_keyusermanual/quickenrolment:showModal');

    return ModalFactory.create({
        type: ModalFactory.types.SAVE_CANCEL,
        large: true,
        title: Str.get_string('enrolusers', 'enrol_keyusermanual'),
    ...
    .then(modal => {
        modal.setSaveButtonText(Str.get_string('enrolusers', 'enrol_keyusermanual'));

        modal.getBodyPromise().then(body => {
            if (body.get(0).querySelector(Selectors.cohortSelector)) {
                modal.setSaveButtonText(Str.get_string('enroluserscohorts', 'enrol_keyusermanual'));
            }
...
const submitFormAjax = (dynamicTable, modal) => {
    ...
    jQuery.ajax(
        `${Config.wwwroot}/enrol/keyusermanual/ajax.php?${form.serialize()}`,
        {
            type: 'GET',
            processData: false,
...
export const init = ({contextid}) => {
    ...
    Prefetch.prefetchStrings('enrol_keyusermanual', [
        'enrolusers',
        'enroluserscohorts',
    ]);
