 ...
 * Enrol candidates.
 */
class enrol_keyusermanual_potential_participant extends user_selector_base {
    protected $enrolid;
    ...
    protected function get_options() {
        ...
        $options['file']    = 'enrol/keyusermanual/locallib.php';
        return $options;
    }
}
 ...
 * Enrolled users.
 */
class enrol_keyusermanual_current_participant extends user_selector_base {
    protected $courseid;
    protected $enrolid;
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_keyusermanual_editselectedusers_operation extends enrol_bulk_enrolment_operation {
    ...
    public function get_title() {
        return get_string('editselectedusers', 'enrol_keyusermanual');
    }
    ...
    public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
        ...
        if (!has_capability("enrol/keyusermanual:manage", $manager->get_context())) {
            return false;
        }
                ...
                foreach ($user->enrolments as $enrolment) {
                    ...
                    $event = \core\event\user_enrolment_updated::create(
                            array(
                                ...
                                'other' => array('enrol' => 'keyusermanual')
                                )
                            );
                    $event->trigger();
                }
     ...
     * @return enrol_keyusermanual_editselectedusers_form
     */
    public function get_form($defaultaction = null, $defaultcustomdata = null) {
        global $CFG;
        require_once($CFG->dirroot.'/enrol/keyusermanual/bulkchangeforms.php');
        return new enrol_keyusermanual_editselectedusers_form($defaultaction, $defaultcustomdata);
    }
}
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_keyusermanual_deleteselectedusers_operation extends enrol_bulk_enrolment_operation {
     ...
     */
    public function get_title() {
        return get_string('deleteselectedusers', 'enrol_keyusermanual');
    }
    ...
    public function get_form($defaultaction = null, $defaultcustomdata = null) {
        ...
        $defaultcustomdata['message'] = get_string('confirmbulkdeleteenrolment', 'enrol_keyusermanual');
        $defaultcustomdata['button'] = get_string('unenrolusers', 'enrol_keyusermanual');
        return new enrol_keyusermanual_deleteselectedusers_form($defaultaction, $defaultcustomdata);
    }
     ...
     */
    public function process(course_enrolment_manager $manager, array $users, stdClass $properties) {
        if (!has_capability("enrol/keyusermanual:unenrol", $manager->get_context())) {
            return false;
        }
        $counter = 0;
        foreach ($users as $user) {
...
/**
 * Migrates all enrolments of the given plugin to enrol_keyusermanual plugin,
 ...
 */
function enrol_keyusermanual_migrate_plugin_enrolments($enrol) {
    global $DB;

    if ($enrol === 'keyusermanual') {
        // We can not migrate to self.
        return;
    }

    $manualplugin = enrol_get_plugin('keyusermanual');
    ...
    $sql = "SELECT e.id, e.courseid, e.status, MIN(me.id) AS mid, COUNT(ue.id) AS cu
         ...
         LEFT JOIN {enrol} me ON (me.courseid = e.courseid AND me.enrol='keyusermanual')
             WHERE e.enrol = :enrol
    ...
    foreach($rs as $e) {
        ...
        if (!$e->mid) {
            ...
            if ($minstance = $DB->get_record('enrol', array('courseid'=>$course->id, 'enrol'=>'keyusermanual'))) {
        ...
        if (!$minstance) {
            ...
            debugging('Failed to find keyuser manual enrolment instance', DEBUG_DEVELOPER);
            continue;
        }
 ...
 */
function enrol_keyusermanual_get_potential_cohorts($context, $enrolid, $search = '', $page = 0, $perpage = 25, $addedenrollment = 0) {
    global $CFG;
